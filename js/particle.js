...
function init() {
	...
	_particles = [];
	_context = document.querySelector("#canvas").getContext('2d');
...
function onResize() {
	...
	_canvasWidth = document.querySelector("#canvas").offsetWidth;
	_canvasHalfWidth = Math.round(_canvasWidth / 2);
	_canvasHeight = document.querySelector("#canvas").offsetHeight,
	_canvasHalfHeight = Math.round(_canvasHeight / 2);

	document.querySelector("#canvas").width = _canvasWidth;
	document.querySelector("#canvas").height = _canvasHeight;
}
		...
		for (j = i + 1; j < _particles.length; j++) {
			...
			if (distance < RADIUS) {
				...
				if (lines <= MAX_LINES) {
					...
					_context.strokeStyle = 'rgba(40,40,40,' + opacity + ')';
					_context.stroke();
				}
			}
		}
